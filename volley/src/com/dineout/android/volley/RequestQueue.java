...
public class RequestQueue {
	...
	private final PriorityBlockingQueue<Request> mNormalNetworkQueue = new PriorityBlockingQueue<Request>();
	private final PriorityBlockingQueue<Request> mOtherNetowkrQueue = new PriorityBlockingQueue<Request>();
	...
	private boolean mThreadingLogicEnabled;
	...
	public RequestQueue(Cache cache, Network network, int threadPoolSize,
			ResponseDelivery delivery,boolean enableThreadingLogic) {
		mCache = cache;
		mNetwork = network;
		mDispatchers = new NetworkDispatcher[threadPoolSize];
		mDelivery = delivery;
		mThreadingLogicEnabled = enableThreadingLogic;
	}
	...
	public GlobalRequestQueueListener getGlobalRequestQueueListener() {
		return globalRequestQueueListener;
	}
	 ...
	 *            Number of network dispatcher threads to create
	 */
	public RequestQueue(Cache cache, Network network, int threadPoolSize,boolean enableThreadingLogic) {
		this(cache, network, threadPoolSize, new ExecutorDelivery(new Handler(
				Looper.getMainLooper())),enableThreadingLogic);
	}
	 ...
	 *            A Network interface for performing HTTP requests
	 */
	public RequestQueue(Cache cache, Network network,boolean enableThreadingLogic) {
		this(cache, network, DEFAULT_NETWORK_THREAD_POOL_SIZE,enableThreadingLogic);
	}
	...
	public void start() {
		...
		mCacheDispatcher = new CacheDispatcher(mCacheQueue, mNetworkQueue,mNormalNetworkQueue,mOtherNetowkrQueue,
				mCache, mDelivery);
		mDelivery.setGlobalRequestQueueListener(globalRequestQueueListener);
		...
		for (int i = 0; i < mDispatchers.length; i++) {
			NetworkDispatcher networkDispatcher = new NetworkDispatcher(
					mNetworkQueue, mNormalNetworkQueue,mOtherNetowkrQueue,mNetwork, mCache, mDelivery,
					(mThreadingLogicEnabled?i:NetworkDispatcher.DEDICATED_DOWNLOAD_MIXED));
			mDispatchers[i] = networkDispatcher;
			networkDispatcher.setGlobalRequestQueueListener(globalRequestQueueListener);
	...
	public boolean hasRequestsWithTag(Object tag) {
		for (Request<?> request : mCurrentRequests) {
	...
	public static interface GlobalRequestQueueListener<T> extends ErrorListener, Listener<T>{
