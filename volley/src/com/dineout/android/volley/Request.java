...
import com.dineout.android.volley.VolleyLog.MarkerLog;
...
public abstract class Request<T> implements Comparable<Request<T>> {
	 ...
	 *
	 * @deprecated Use
	 * {@link #Request(int, String, com.dineout.android.volley.Response.ErrorListener)}
	 * .
	 */
	@Deprecated
	...
	public Request(int method, String url, Response.ErrorListener listener) {
		...
		int defaultTrafficStatsTag = 0;
		try {
			defaultTrafficStatsTag =
					TextUtils.isEmpty(url) ? 0 : Uri.parse(url).getHost().hashCode();
		} catch (Exception e) {
			defaultTrafficStatsTag = 0;
		}
		mDefaultTrafficStatsTag = defaultTrafficStatsTag;
	}
	...
	public String getUrl() {
		return TextUtils.isEmpty(mRedirectUrl) ? this.mUrl : mRedirectUrl;
	}
	...
	public String getCacheKey() {
		...
		return getUrl() + getBodyString() + this.headers.toString();
	}
	...
	abstract protected Response<T> parseNetworkResponseUnpacked(NetworkResponse response);


	final protected Response<T> parseNetworkResponse(NetworkResponse response) {
		final boolean gzipEnabled;
		if (response.headers != null) {
			String gzipHeader = response.headers.get("Content-Encoding");
			gzipEnabled = gzipHeader != null
					&& "gzip".equalsIgnoreCase(gzipHeader);
		} else {
			gzipEnabled = false;
		}
		...
		if (gzipEnabled) {
			try {
				GZIPInputStream gzipInputStream = new GZIPInputStream(
						new ByteArrayInputStream(response.networkData));
				ByteArrayOutputStream bos = new ByteArrayOutputStream();
				byte[] b = new byte[1024];
				int read = 0;
				while ((read = gzipInputStream.read(b)) != -1) {
					bos.write(b, 0, read);
				}
				bos.flush();
				response.processedData = bos.toByteArray();
				bos.close();
				gzipInputStream.close();
			} catch (IOException e) {
				e.printStackTrace();
				response.processedData = response.networkData;
			}
		} else {
			response.processedData = response.networkData;
		}
		Response<T> parseNetworkResponseUnpacked = parseNetworkResponseUnpacked(
				response);
		parseNetworkResponseUnpacked.dataSize = response.networkData != null
				? response.networkData.length
				: 0;
		response.networkData = null;
	 ...
	 */
	abstract protected void deliverResponse(Request<T> request, T response, Response<T> fullResponse);
	 ...
	 */
	public void deliverError(Request<T> request, VolleyError error) {
		if (this.mErrorListener != null) {
			this.mErrorListener.onErrorResponse(request, error);
		}
	}
	...
	public int compareTo(Request<T> other) {
		...
		// ordering.
		return deleteCache ? 10 : (left == right ? this.mSequence - other.mSequence :
	...
	public String toString() {
		...
		return (this.mCanceled ? "[X] " : "[ ] ") + getUrl() + " " + trafficStatsTag + " " +
				getPriority() + " " + this.mSequence;
	}
}
