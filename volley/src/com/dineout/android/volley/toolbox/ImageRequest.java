...
public class ImageRequest extends Request<Bitmap> {
	...
	protected Response<Bitmap> parseNetworkResponseUnpacked(NetworkResponse response) {
		...
		synchronized (sDecodeLock) {
			try {
				...
				VolleyLog.e("Caught OOM for %d byte image, url=%s",
						response.processedData.length, getUrl());
	...
	private Response<Bitmap> doParse(NetworkResponse response) {
		byte[] data = response.processedData;
	...
	@Override
	protected void deliverResponse(Request<Bitmap> request, Bitmap response,
			Response<Bitmap> fullRespons) {
		mListener.onResponse(request, response, fullRespons);
	}
