...
import com.dineout.android.volley.VolleyLog;
...
public class DiskBasedCache implements Cache {
	 ...
	 *            The root directory of the cache.
	 */
	public DiskBasedCache(File rootDirectory, boolean enableCompression) {
		this(rootDirectory, DEFAULT_DISK_USAGE_BYTES, enableCompression);
	}
	...
	public synchronized Entry get(String key) {
		...
		try {
			...
			if (this.mEnableCompression)
				is = new GZIPInputStream(is);
			cis = new CountingInputStream(is);
			CacheHeader.readHeader(file, cis); // eat header
			byte[] data = streamToBytes(cis, (int) (entry.size));
	...
	public synchronized void initialize() {
		...
		for (File file : files) {
			...
			try {
				...
				CacheHeader entry = CacheHeader.readHeader(file, fis);
	...
	public synchronized void put(String key, Entry entry) {
		...
		try {
			...
			CacheHeader e = new CacheHeader(key, file, entry);
	...
	private String getFilenameForKey(String key) {
		...
		String localFilename = String
				.valueOf(key.substring(0, firstHalfLength).hashCode());
		localFilename += String
				.valueOf(key.substring(firstHalfLength).hashCode());
		return VERSION + localFilename;
	}
	...
	private void putEntry(String key, CacheHeader entry) {
		if (!mEntries.containsKey(key)) {
			mTotalSize += entry.file.length();
		} else {
			CacheHeader oldEntry = mEntries.get(key);
			mTotalSize += (entry.file.length() - oldEntry.file.length());
		}
	...
	private static byte[] streamToBytes(InputStream in, int length)
		...
		if (pos != length) {
			throw new IOException(
					"Expected " + length + " bytes, read " + pos + " bytes");
		}
		return bytes;
	}
	...
	static class CacheHeader {
		 ...
		 *            The cache entry.
		 */
		public CacheHeader(String key, File file, Entry entry) {
			this.key = key;
		 ...
		 * @throws IOException
		 */
		public static CacheHeader readHeader(File file, InputStream is)
				throws IOException {
			CacheHeader entry = new CacheHeader(file);
