...
import com.dineout.android.volley.Request.Priority;
...
public class ImageLoader {
	 ...
	 * responses.
	 */
	private int mBatchResponseDelayMs = 50;
	...
	private final HashMap<String, BatchedImageRequest> mBatchedResponses = new LinkedHashMap<String, BatchedImageRequest>();
	 ...
	 *            loaded.
	 */
	public ImageContainer get(ImageUrl requestUrl, final ImageListener listener) {
	 ...
	 */
	public ImageContainer get(final ImageUrl requestUrl,final ImageListener imageListener,
			final int maxWidth,final int maxHeight,boolean requestHighResolution) {
		// only fulfill requests that were initiated from the main thread.
		throwIfNotOnMainThread();
		String cacheKeyString = getCacheKey(requestUrl.url, maxWidth, maxHeight);
		List<String> cacheKeyList = null;
		if(requestUrl.otherResolutionUrlslist!=null) {
			 cacheKeyList = new LinkedList<String>();
			 int index = 0;
			for(ImageUrl otherResolution : requestUrl.otherResolutionUrlslist) { 
				final String cacheKey = getCacheKey(otherResolution.url, maxWidth, maxHeight);
				...
				if (cachedBitmap != null) {
					...
					ImageContainer container = new ImageContainer(cachedBitmap,
							otherResolution.url, null, null);
					imageListener.onResponse(container, false);
					ImageUrl highResolutionImage = getHighResolutionImage(requestUrl.otherResolutionUrlslist, maxWidth, maxHeight);
					if(otherResolution!= highResolutionImage && requestUrl.otherResolutionUrlslist.indexOf(highResolutionImage) < index) {
						ImageContainer imageContainer = get(highResolutionImage, imageListener, maxWidth, maxHeight, true);
						imageContainer.dependentContainer = container;
					}
			...
			Bitmap cachedBitmap = mCache.getBitmap(cacheKeyString);
			if (cachedBitmap != null) {
				// Return the cached bitmap.
				ImageContainer container = new ImageContainer(cachedBitmap,
		...
		final ImageContainer imageContainer = new ImageContainer(null, requestUrl.url,
				cacheKeyString, imageListener);
		...
		if(cacheKeyList!=null) {
			...
			for(String cacheKey : cacheKeyList) {
				...
				if (request != null) {
					...
					ImageUrl highResolutionImage = getHighResolutionImage(requestUrl.otherResolutionUrlslist, maxWidth, maxHeight);
					if(requestUrl.otherResolutionUrlslist.indexOf(highResolutionImage) < index) {
						ImageContainer ic = get(highResolutionImage, imageListener, maxWidth, maxHeight, true);
						ic.dependentContainer = ic;
					}
			...
			BatchedImageRequest request = mInFlightRequests.get(cacheKeyString);
			if (request != null) {
				// If it is, add this request to the list of listeners.
				request.addContainer(imageContainer);
				return imageContainer;
		...
		ImageUrl finalUrl = requestUrl;
		boolean iscached = false;
		if(requestUrl.otherResolutionUrlslist != null) {
			for(ImageUrl url : requestUrl.otherResolutionUrlslist) {
				if(mRequestQueue.getCache().isCached(url.url)) {
					finalUrl = url;
					iscached = true;
					break;
				}
		...
		final String cacheKey = getCacheKey(finalUrl.url, maxWidth, maxHeight);
		// The request is not already in flight. Send the new request to the
		// network and
		// track it.
		Request<?> newRequest = new ImageRequest(finalUrl.url,
				new Listener<Bitmap>() {
					
					@Override
					public void onResponse(Request<Bitmap> request,
							Bitmap response, Response<Bitmap> fullResponse) {
						if(imageContainer.dependentContainer==null || !imageContainer.dependentContainer.cancelled) {
							onGetImageSuccess(cacheKey, response);
							List<ImageUrl> otherResolutionUrlslist = requestUrl.otherResolutionUrlslist;
							if(!imageContainer.cancelled &&  otherResolutionUrlslist!=null && maxWidth>0 && maxHeight > 0 ) {
								ImageUrl highResolutionUrl = getHighResolutionImage(otherResolutionUrlslist, maxWidth, maxHeight);
								if(!highResolutionUrl.url.equals(request.getUrl())) {
									get(highResolutionUrl, imageListener, maxWidth, maxHeight,true);
								}
							}
						}
					}
	...
	ImageUrl getHighResolutionImage(List<ImageUrl> otherResolutionUrlslist,int maxWidth,int maxHeight) {
		ImageUrl current;
		ImageUrl previous = null;
		ImageUrl highResolutionUrl= null;
		
		for (int i = 0 ; i < otherResolutionUrlslist.size() ; i++) {
			highResolutionUrl = current = otherResolutionUrlslist.get(i);
			...
			if(current.width > current.height) {
				if(current.width < maxWidth) {
					if(previous!=null) {
						if((maxWidth-current.width) < (previous.width-maxWidth)) {
							highResolutionUrl = current;
						}
						else {
							highResolutionUrl = previous;
						}
					}
					else {
						highResolutionUrl = current;
				...
				if(current.height < maxHeight) {
					if(previous!=null) {
						if((maxHeight-current.height) < (previous.height-maxHeight)) {
							highResolutionUrl = current;
						}
						else {
							highResolutionUrl = previous;
	...
	public class ImageContainer {
		...
		private ImageContainer dependentContainer;
	...
	private void batchResponse(String cacheKey, BatchedImageRequest request) {
		...
		if (mRunnable == null) {
			mRunnable = new Runnable() {
				...
				@Override
				public void run() {
					Set<String> keySet = mBatchedResponses.keySet();
					for (String key : keySet) {
						BatchedImageRequest bir = mBatchedResponses.get(key);
						for (ImageContainer container : bir.mContainers) {
							...
							if (bir.getError() == null) {
								...
								container.mListener.onErrorResponse(null,
										bir.getError());
							}
						}
					}
	...
	public static class ImageUrl {
		...
		String url;
		List<ImageUrl> otherResolutionUrlslist;
		int width;
		int height;
		String originalUrl;
		...
		public ImageUrl(String url,List<ImageUrl> otherResolutionUrlslist,int width, int height) {
			this.originalUrl = this.url = url;
			this.width = width;
			this.height = height;
			this.otherResolutionUrlslist = otherResolutionUrlslist;
			if(otherResolutionUrlslist!=null) {
				for(ImageUrl otherUrl : otherResolutionUrlslist) {
					otherUrl.originalUrl = url;
				}
			}
		}
